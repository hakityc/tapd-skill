...
在进入 A-F 工作流前，解析本轮工作项或恢复当前 Git 分支绑定。不要让旧分支 context 覆盖用户本轮明确提供的新工作项。
...
2. **本轮 TAPD URL**：P0 支持标准 `tapd_fe/<workspace>/<story|task|bug>/detail/<id>`。
...
Story/Task 只有 URL 且缺标题或用户时，使用能力矩阵中的 MCP 读取工具补全；失败就保持缺失，不伪造。Bug 缺少读取能力时只允许本地绑定，不进入完整 intake。
...
3. `CONTEXT_ALREADY_BOUND` 时展示旧、新工作项并等待确认。
4. 只有用户确认覆盖后才重试 `bind ... --force`。
5. 成功后进入 MCP 能力门禁；Story/Task 可进入 intake，Bug 按可选能力处理。
...
不得自动修改 `.gitignore`、pull、stash 或提交这些文件。
