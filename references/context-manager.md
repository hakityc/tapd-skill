...
Story/Task 只有 URL 且缺标题时用 `get_stories_or_tasks` 补全，Bug 使用 `get_bug` 补全。准备执行 `start` 时，若标题仍缺失或标题无法产生英文 slug，skill 应根据已读取的标题生成简短英文 `--slug`；MCP 不可用时才降级为 ID slug。不得伪造标题或用户。
...
6. 成功后进入 MCP 能力门禁；Story/Task/Bug 均进入对应 intake。
