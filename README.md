...
- 从 TAPD Context JSON 或 Story/Task/Bug URL 开始工作。
- 支持 TAPD 页面常见的 Story/Task prong view 和 Bug bugtrace view 链接。
...
- TAPD 远端读写统一通过 MCP，Skill 不直接处理 token 或调用 OpenAPI。
...
- Story/Task/Bug context 与 intake 主路径
- 首次使用生成 `.tapd/config.json`
- `init/configure/start/bind/current/status`
- 标准 detail、Story/Task prong view 与 Bug bugtrace view 链接
- Task → 父 Story → 同级 tasks intake
- Bug 复现、影响、附件/评论 gap、关联需求与回归范围
- MCP 评论回读和 Story/测试用例关联（按运行时能力门禁）
- 日常编码执行、继续开发和开发收尾
