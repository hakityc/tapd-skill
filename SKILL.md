...
- **C. 任务编排**：创建任务、回填 owner/description，并默认串联 F 自动估时、排期和写回。
...
- 用户说“拆任务 / 创建任务 / 回填 TAPD / 建分支拆任务”时：先走 B intake，再走 C 创建/回填任务，最后自动走 F 估时排期并写回本人任务的 effort/begin/due。
- 用户单独说“估时 / 排期 / 补工时”时：只走 F。
- 用户明确说“不写工时 / 只拆任务 / 仅创建任务不排期”时：C 跳过 F，effort/begin/due 保持空或仅按用户指定字段处理。
...
- C：`references/task-orchestrator.md`，默认在创建/回填后串联 `references/effort-scheduler.md`
